...
class Ion2Vector:
    ...
    def CheckFixMod_fixno(self, peptide, modlist):
        return True
        
    def CheckFixMod(self, peptide, modlist):
        return self.CheckFixMod_fixno(peptide, modlist)
    ...
    def FeaturizeOnePeptide(self, peptide, modinfo, charge, padding_zero = False):
        ...
        if not self.CheckFixMod(peptide, modlist): return None
    ...
    def Featurize_buckets_predict(self, ion_file, nce, instrument):
    ...
    def Featurize_buckets(self, ion_file, nce, instrument):
        ...
        while True:
            ...
            for ion_type in self.config.GetIonTypeNames():
                ...
                peaks = peaks.strip().strip(";").split(";")
                type2inten.update(dict([(peak[:peak.find(',')], float(peak[peak.find(',')+1:])) for peak in peaks]))
